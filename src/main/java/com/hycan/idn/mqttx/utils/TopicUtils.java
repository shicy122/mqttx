...
public class TopicUtils {
    ...
    /** 客户端上下线主题 */
    private static final Pattern SYS_EVENT_TOPIC = Pattern.compile("^\\$SYS/broker/(.+)/clients/(.+)");
    //@formatter:on
     ...
     * @return true 如果被授权
     */
    public static boolean hasAuthToSubTopic(Channel channel, String topic) {
        return hasAuth(channel, topic, AbstractMqttSessionHandler.AUTHORIZED_SUB_TOPICS);
    }
     ...
     * @return true 如果被授权
     */
    public static boolean hasAuthToPubTopic(Channel channel, String topic) {
        return hasAuth(channel, topic, AbstractMqttSessionHandler.AUTHORIZED_PUB_TOPICS);
    }
    ...
    private static boolean hasAuth(Channel channel, String topic, String type) {
        ...
        for (String authorizedTopic : authorizedTopics) {
            if (ALL_TOPIC.equals(authorizedTopic) || TopicUtils.match(topic, authorizedTopic)) {
                return true;
            }
        }
    ...
    public static boolean isSys(String topic) {
        return topic != null && (topic.startsWith(SYS_TOPIC) || topic.startsWith(SHARE_TOPIC + "/" +SYS_TOPIC));
    }

    /**
     * 客户端上/下线 topic
     * @param topic 主题
     * @return true, if topic is sys connect/disconnect
     */
    public static boolean isSysEvent(String topic) {
        if (isShare(topic)) {
            topic = topic.substring(TopicUtils.SHARE_TOPIC.length() + 1);
        }
        return SYS_EVENT_TOPIC.matcher(topic).matches();
    }
     ...
     *
     * @param topic 主题
     * @return true, if topic is wildcard
     */
    ...
    public static boolean match(String pub, String sub) {
        // 如果 topic 含 $share 前缀，则截取掉，再进行匹配
        if (TopicUtils.isShare(sub)) {
            sub = sub.substring(TopicUtils.SHARE_TOPIC.length() + 1);
        }
